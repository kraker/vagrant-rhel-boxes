...
For RHEL guest registration, prefer `RHSM_ORG` + `RHSM_ACTIVATIONKEY` for repeatable use. For local one-off use, rely on the `vagrant-registration` interactive prompt rather than adding password templates or hardcoded credentials.

Before committing, run the relevant lightweight checks:

- `/opt/vagrant/embedded/bin/ruby -c Vagrantfile` when changing `Vagrantfile`.
...
**`build.yml`** runs the playbook on the self-hosted runner. Triggered only by `workflow_dispatch` (manual test) and pushes of `v*` tags (releases) — pushes to `main` do *not* build, by design, since builds are slow and gated behind a real release. The `build` job is a `provider × distro` matrix (`[libvirt, virtualbox] × [rhel-10.0]`) with `fail-fast: false`, so a virtualbox failure doesn't abort the libvirt leg. Each leg uploads a per-provider artifact (`<distro>-vagrant-<provider>`) for inspection / debugging. The `publish` job (`needs: build`, gated on `startsWith(github.ref, 'refs/tags/v')`) runs the same matrix, downloads the artifact, derives the box family from `distro` (`rhel-10.0` → `rhel-10`), pulls the version from the tag (`${GITHUB_REF#refs/tags/v}`), and shells out to `vagrant cloud publish kraker/<family> <version> <provider> ...` once per provider. Auth is HCP service-principal via repo secrets `HCP_CLIENT_ID` + `HCP_CLIENT_SECRET` (Vagrant >= 2.4.4 mints HCP tokens itself from those env vars — no `hcp` CLI bootstrap needed). Workflow-level `concurrency: image-builder-${{ github.ref }}` keeps back-to-back tags or dispatches from racing the matrix against itself.
